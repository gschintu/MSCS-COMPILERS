...
cast_Rval (struct sem_rec *left, struct sem_rec *right)
{
  ...
  if ((left->s_type & T_DOUBLE) && !(right->s_type & T_DOUBLE))
      R = Builder.CreateSIToFP(R, Type::getDoubleTy(TheContext), "cast1");
  else if ((left->s_type & T_INT) && !(right->s_type & T_INT))
      R = Builder.CreateFPToSI(R, Type::getInt32Ty(TheContext), "cast2");
  return R;
}
...
{
  //backpatch(e1->s_bb->s_true, m);

  //return node((void *) NULL,(void *) NULL, 0,(struct sem_rec*) NULL,((struct sem_rec*)e2->s_bb)->s_true, ((struct sem_rec*)e1->s_bb)->s_false);

  //node(void *value, void *bb, int type, struct sem_rec *link,struct sem_rec *true_rec, struct sem_rec *false_rec)
...
{
  //return node(0, 0, e->s_false, e->s_true);
...
set(const char *op, struct sem_rec *x, struct sem_rec *y)
{
  ...
  value = cast_Rval(x,y);
/*
  ty = llvm::dyn_cast<PointerType>(((Value*)value)->getType()
                                  ->getScalarType())->getElementType();
*/
...
genstring(char *s)
{
  ...
  ret = s_node(parse_escape_chars(s), T_STR); // maybe needs identifier like a currtemp
  ret->s_value = (void*) Builder.CreateGlobalStringPtr(parse_escape_chars(s), parse_escape_chars(s));
  return ret;  
