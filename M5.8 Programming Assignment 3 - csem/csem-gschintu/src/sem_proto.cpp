...
struct sem_rec* ccexpr(struct sem_rec *R)
{
  ...
  if (e->s_type == T_INT) {
    ...
    Value *cmp = Builder.CreateICmpNE((Value*)R->s_value, zero);
    val = Builder.CreateCondBr(cmp, tmp_true, tmp_false);
  }
  else if (e->s_type == T_DOUBLE) {
    // Create a float comparison with zero
    Value *zero = ConstantFP::get(Type::getFloatTy(Context), 0);
    Value *cmp = Builder.CreateFCmpONE((Value*)R->s_value, zero);
    val = Builder.CreateCondBr(cmp, tmp_true, tmp_false);
  }
  else {
...
indx (struct sem_rec *x, struct sem_rec *i)
{
    ...
    R = node(currtemp(), x->s_mode, NULL, dodo);
